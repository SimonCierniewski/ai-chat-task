...
export const memoryRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  // POST /api/v1/memory/import - Import conversation messages for testing
  fastify.post('/import', {
    schema: {
      summary: 'Import conversation messages',
      description: 'Import conversation messages to Zep and optionally test memory retrieval',
      tags: ['Memory'],
      body: Type.Object({
        ...
        testMemory: Type.Optional(Type.Boolean())
      }),
      response: {
        200: Type.Object({
          ok: Type.Literal(true),
          role: Type.String(),
          memoryContext: Type.Optional(Type.String()),
          memoryMs: Type.Optional(Type.Number())
        }),
        400: Type.Object({
          error: Type.String(),
          message: Type.String()
    ...
    const startTime = Date.now();
    const { sessionId, message, testMemory } = request.body;
    const userId = sessionId; // Use sessionId as userId for testing
    
    try {
      logger.info(`Importing message to Zep`, { sessionId, role: message.role, testMemory });
      
      // Initialize Zep adapter
      const zepAdapter = ZepAdapter.getInstance();
      
      // Ensure user and thread exist
      await zepAdapter.ensureUser(userId);
      await zepAdapter.ensureThread(userId, sessionId);
      
      // Add the message to Zep
      const zepMessage: Message = {
        role: message.role,
        content: message.content,
        role_type: message.role === 'user' ? 'user' : 'assistant'
      };
      
      await zepAdapter.client.thread.addMessages(sessionId, {
        messages: [zepMessage]
      });
      
      logger.info(`Message added to Zep thread ${sessionId}`);
      
      // Optionally test memory retrieval
      let memoryContext: string | undefined;
      let memoryMs: number | undefined;
      
      if (testMemory && message.role === 'user') {
        const memoryStartTime = Date.now();
        const context = await zepAdapter.getContextBlock(sessionId, { mode: 'basic' });
        memoryMs = Date.now() - memoryStartTime;
        memoryContext = context.results;
        logger.info(`Memory retrieved in ${memoryMs}ms`, { sessionId });
      }
      
      const totalMs = Date.now() - startTime;
      logger.info(`Import completed in ${totalMs}ms`);
      ...
      logger.error('Error importing message:', error);
      return reply.status(500).send({
        error: 'IMPORT_ERROR',
